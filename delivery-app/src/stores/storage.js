...
export async function addMotoClient(client) {
  try {
    ...
    // Sync to Supabase in background (don't await, let React handle optimistic UI)
    const clientData = {
      user_id: userId,
...
export async function deleteMotoClient(clientId) {
  try {
    ...
    // Delete from Supabase in background (don't await, let React handle optimistic UI)
    supabase
      .from('moto_clients')
      .delete()
      .eq('id', clientId)
